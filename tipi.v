...
Section RazneFunkcije.
  ...
  Definition vaja1_8 : A -> unit * A :=
    fun (a : A) => (tt, a).  (* edini elelement tipa unit, v coqu se mu rece tt *)
...
Section Izomorfizmi1.
  ...
  Proof.
    ...
    exists 
        ...
        (fun a: A => (tt, a)).
    split.
    - intro x. (* Dobimo, da je x urejeni par. Toda urejeni par pa lahko razstavimo: *)
      destruct x. (* To dvoje lahko naredis tudi z intros [u a]. Ali pa kar intros [[] a]. *)
      simpl.
      destruct u.
      reflexivity.
    - intro a.
...
Najprej ga definirajmo. *)
  ...
  Proof.
    unfold iso.
    exists
      (fun (h : unit -> A) => h tt), (*h je input, je funkcija iz unit v A. z fun pa ta h slikamo v h(tt). *)
      (fun (a : A)(x : unit) => a). (* lah tut (_ : unit), saj ti x nikjer v sliki ne nastopa, saj slikas kar v a. *)
    split.
    - intro h.
      apply F. (* Ce reflexivity ne dela, ponavadi skusis uporabiti funkcijsko ekstenzionalnost. Pri nas je to kar 'F'. *)
      intros []. (*  NIC stvari smo uvedli, ker itak vemo, da bo x mogu bit kar tt. *)
      reflexivity.
    - auto.
  Qed.
...
Print vaja3_3'. (* Tokrat dobimo lepse / malo drugace. *)



(* Ce hoces videt, kako se kej dokaze:
Proof.
  tauto.
Defined.
Print vaja3_3. *)
